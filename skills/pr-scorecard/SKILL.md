---
name: pr-scorecard
description: "Use to score a pull request for review — a scorecard rating it on correctness, impact, alignment, and security so a reviewer knows where to focus. Trigger: 'score this PR', 'scorecard for PR #123', 'triage this PR', 'classify PR #123', 'how risky is this PR', 'is this PR safe to merge', 'where should I focus reviewing this PR', 'summarize the review risk of this change'. Also trigger when someone is about to review a PR and wants a fast read on it before diving in. This is a SCORECARD / triage layer that routes attention — it does NOT replace deep review; it flags when a dedicated security or line-by-line review is required. Do NOT trigger for writing code, fixing bugs, or when the user wants a full code review rather than a fast classification."
---
...
## Handoff
- <State what deeper review is required and why — e.g. "Security ≥ Medium: a dedicated security review is required before merge"; "recommend a full line-by-line review of the migration". If a review command/agent is available in this environment (e.g. `/security-review`, `/code-review`), name it as the way to do it; otherwise just tell the reviewer it's required. Note anything blocking.>
